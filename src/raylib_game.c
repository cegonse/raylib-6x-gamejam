...
static void gameFrame(void)
{
    ...
    if (player_y_lerp_t > 0.9f && current_anim == ANIM_SPIN) {
        current_anim = ANIM_DANCE;
        animation_frame = 0;
    ...
    if (player_y_lerp_t > 0.9f && player_hexagon_x == trinket_hexagon_x && player_hexagon_y == trinket_hexagon_y) {
        points++;
        time += 5.0f;
...
static void render(void)
{
    ...
    if (game_state == GAME_STATE_INIT)
    {
        ...
        logo_c1 = lerpHue(logo_c1);
        logo_c2 = lerpHue(logo_c2);
        logo_c3 = lerpHue(logo_c3);
    ...
    if (game_state == GAME_STATE_INIT || game_state == GAME_STATE_END)
    {
        ...
        {
            start_hover_color = lerpHue(start_hover_color);
